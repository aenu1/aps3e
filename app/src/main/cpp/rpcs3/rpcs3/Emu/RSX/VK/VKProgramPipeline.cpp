...
namespace vk
{
	...
	namespace glsl
	{
		...
		VkShaderModule shader::compile()
		{
			...
			_vkCreateShaderModule(*g_render_device, &vs_info, nullptr, &m_handle);
		...
		void shader::destroy()
		{
			...
			if (m_handle)
			{
				_vkDestroyShaderModule(*g_render_device, m_handle, nullptr);
				m_handle = nullptr;
			}
		}
		...
		program::~program()
		{
			_vkDestroyPipeline(m_device, m_pipeline, nullptr);

			if (m_pipeline_layout)
			{
				_vkDestroyPipelineLayout(m_device, m_pipeline_layout, nullptr);
		...
		program& program::link(bool separate_objects)
		{
			...
			if (is_graphics_pipe)
			{
				...
				CHECK_RESULT(_vkCreateGraphicsPipelines(m_device, g_pipeline_cache, 1, &create_info, nullptr, &m_pipeline));
			}
			else
			{
				VkComputePipelineCreateInfo create_info = *p_compute_info;
				create_info.layout = m_pipeline_layout;
				CHECK_RESULT(_vkCreateComputePipelines(m_device, g_pipeline_cache, 1, &create_info, nullptr, &m_pipeline));
			}
		...
		void program::create_pipeline_layout()
		{
			...
			CHECK_RESULT(_vkCreatePipelineLayout(m_device, &create_info, nullptr, &m_pipeline_layout));
		}
		...
		void descriptor_table_t::destroy()
		{
			...
			{
				_vkDestroyDescriptorSetLayout(m_device, m_descriptor_set_layout, nullptr);
			}
		...
		void descriptor_table_t::create_descriptor_template()
		{
			auto push_descriptor_slot = [this](unsigned idx, VkWriteDescriptorSet& writer)
			{
				...
				if (auto ptr = std::get_if<VkDescriptorSubBufferEx>(&slot))
				{
                    const VkDescriptorBufferInfo buffer_info = { ptr->buffer, 0, VK_WHOLE_SIZE };
					m_descriptor_set.push(buffer_info, type, idx);
					return;
				}
		...
		void descriptor_table_t::update_descriptor_template()
		{
			auto update_descriptor_slot = [this](unsigned idx)
			{
				...
				{
                    const VkDescriptorBufferInfo buffer_info = { ptr->buffer, 0, VK_WHOLE_SIZE };
                    m_descriptor_template[idx].pBufferInfo = m_descriptor_set.store(buffer_info);
					return;
				}
