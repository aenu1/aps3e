...
namespace vk
{
	...
	void physical_device::get_physical_device_features(bool allow_extensions)
	{
		...
		_vkGetPhysicalDeviceFeatures2(dev, &features2);
...
#undef SET_DESCRIPTOR_BITFLAG
	...
	void physical_device::get_physical_device_properties_0(bool allow_extensions)
	{
		...
		_vkGetPhysicalDeviceProperties2(dev, &properties2);
	}
	...
	const VkQueueFamilyProperties& physical_device::get_queue_properties(u32 queue)
	{
		if (queue_props.empty())
		{
			...
			_vkGetPhysicalDeviceQueueFamilyProperties(dev, &count, queue_props.data());
		}
...
#undef SET_DESCRIPTOR_BITFLAG
		...
		if (auto error = _vkCreateDevice(*pgpu, &device, nullptr, &dev))
		{
		...
		// Initialize queues
		_vkGetDeviceQueue(dev, graphics_queue_idx, 0, &m_graphics_queue);
		_vkGetDeviceQueue(dev, transfer_queue_idx, transfer_queue_sub_index, &m_transfer_queue);

		if (present_queue_idx != umax)
		{
			_vkGetDeviceQueue(dev, present_queue_idx, 0, &m_present_queue);
		}
	...
	const VkFormatProperties render_device::get_format_properties(VkFormat format) const
	{
		...
		_vkGetPhysicalDeviceFormatProperties(*pgpu, format, &props);
		return props;
	}
...
#undef TEST_VK_FEATURE
	...
	memory_type_mapping get_memory_mapping(const vk::physical_device& dev)
	{
		...
		_vkGetPhysicalDeviceMemoryProperties(pdev, &memory_properties);
	...
	gpu_formats_support get_optimal_tiling_supported_formats(const physical_device& dev)
	{
		const auto test_format_features = [&dev](VkFormat format, VkFlags required_features, VkBool32 linear_features) -> bool
		{
			...
			_vkGetPhysicalDeviceFormatProperties(dev, format, &props);
