...
VKGSRender::VKGSRender(utils::serial* ar) noexcept : GSRender(ar)
{
    ...
    vk::init_instance_pfn(m_instance);
	m_instance.bind();
	...
	null_buffer_view = vk::buffer_upload::create(*m_device,*null_buffer,VK_FORMAT_R8_UINT, 0, 32);
	...
	for (u32 i = 0; i < m_swapchain->get_swap_image_count(); ++i)
	{
		...
		_vkCmdClearColorImage(*m_current_command_buffer, target_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear_color, 1, &range);
...
void VKGSRender::bind_viewport()
{
	...
	_vkCmdSetViewport(*m_current_command_buffer, 0, 1, &m_viewport);
	_vkCmdSetScissor(*m_current_command_buffer, 0, 1, &m_scissor);
}
...
void VKGSRender::clear_surface(u32 mask)
{
	...
	{
		begin_render_pass();
		_vkCmdClearAttachments(*m_current_command_buffer, ::size32(clear_descriptors), clear_descriptors.data(), 1, &region);
	}
}
...
bool VKGSRender::release_GCM_label(u32 type, u32 address, u32 args)
{
	...
	{
		_vkCmdUpdateBuffer(*m_current_command_buffer, mapping.second->value, mapping.first, 4, &write_data);
		flush_command_queue();
	}
	else
	{
		...
		cmd->begin();
		_vkCmdUpdateBuffer(*cmd, mapping.second->value, mapping.first, 4, &write_data);
		_vkCmdUpdateBuffer(*cmd, m_host_object_data->value, ::offset32(&vk::host_data_t::commands_complete_event), 8, &release_event_id);
		cmd->end();
...
void VKGSRender::on_guest_texture_read(const vk::command_buffer& cmd)
{
	...
	_vkCmdUpdateBuffer(cmd, m_host_object_data->value, ::offset32(&vk::host_data_t::texture_load_complete_event), sizeof(u64), &event_id);
}
...
void VKGSRender::close_and_submit_command_buffer(vk::fence* pFence, VkSemaphore wait_semaphore, VkSemaphore signal_semaphore, VkPipelineStageFlags pipeline_stage_flags)
{
	...
	{
		_vkCmdUpdateBuffer(*m_current_command_buffer,
...
void VKGSRender::begin_conditional_rendering(const std::vector<rsx::reports::occlusion_query_info*>& sources)
{
	...
	if (!m_cond_render_buffer)
	{
		...
		_vkCmdFillBuffer(*m_current_command_buffer, m_cond_render_buffer->value, 4, 4, 0xFFFFFFFFu);
	...
	else if (num_hw_queries > 0)
	{
		...
		if (!partial_eval)
		{
			...
			_vkCmdFillBuffer(*m_current_command_buffer, m_cond_render_buffer->value, 0, 4, 0);
